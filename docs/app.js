...
const TRANSLATIONS = {
  'pt-BR': {
    ...
    nav_extension: 'Extensão & Studio',
    nav_mcp: 'Servidor MCP (IA)',
    ...
    sidebar_extension: 'Extensão & Studio',
    sidebar_mcp: 'Servidor MCP & IA',
    sidebar_port_discovery: 'Detecção Automática de Portas',
    ...
    ext_tag: 'Zero Código & Studio',
    ext_title: 'Extensão para Google Chrome & Edge + BackOverrides Studio',
    ext_desc: 'Interceptação na camada de rede, gerador visual de content-script e reload facilitado da extensão sem abrir configurações do navegador.',
    ...
    ext_c4_title: 'BackOverrides Studio',
    ext_c4_desc: 'Página de configuração visual (<code>options.html</code>) embutida na extensão. Permite criar regras interativas, editar padrões regex e gerar o script in-page com 1 clique.',
    ext_c5_title: 'Preservação de Authorization',
    ...
    mcp_title: 'Model Context Protocol (MCP) Server',
    mcp_desc: 'Conecte seu assistente de IA (GitHub Copilot no VS Code, Claude Desktop, Cursor ou Gemini) ao BackOverrides para inspecionar, testar e configurar regras por linguagem natural.',
    mcp_vscode_tip: 'Após salvar o arquivo, abra o <strong>GitHub Copilot Chat</strong> no VS Code e seu assistente terá acesso imediato a todas as ferramentas do BackOverrides.',
    mcp_tools_title: 'Ferramentas Expostas pelo Servidor MCP (8 Tools)',
    mcp_tools_desc: 'Seu assistente de IA ganha superpoderes para manipular o tráfego da sua máquina de forma segura:',
    mcp_tool_status_desc: 'Verifica se o proxy local está online, porta ativa (com fallback automático), PID e total de requisições interceptadas.',
    mcp_tool_rules_desc: 'Retorna a lista completa de rotas configuradas, métodos permitidos e o alvo resolvido no localhost.',
    mcp_tool_add_desc: 'Adiciona uma nova regra ao <code>back-overrides.json</code> programaticamente sem necessidade de edição manual.',
    mcp_tool_toggle_desc: 'Ativa ou desativa temporariamente uma regra por índice ou rota para comparar chamadas com e sem interceptação.',
    mcp_tool_test_desc: 'Simula como uma requisição (URL + método HTTP) será tratada: match de regra, resolução do destino local e headers CORS.',
    mcp_tool_browser_desc: 'Lança uma instância do Chrome, Edge ou Brave com a extensão do BackOverrides automaticamente carregada.',
    mcp_tool_snippets_desc: 'Gera snippets rápidos para injeção via DevTools console, tag <code>&lt;script&gt;</code> no HTML ou import npm.',
    mcp_tool_cfg_desc: 'Configura automaticamente o arquivo <code>.vscode/mcp.json</code> no workspace atual.',
    mcp_chat_title: 'Exemplo Prático: Diálogo com GitHub Copilot no VS Code',
    mcp_chat_desc: 'Veja como o assistente utiliza o MCP para diagnosticar e corrigir um problema de rota em segundos:',
    mcp_chat_msg1: 'Copilot, a rota <code>/bff/core/v1/oauth2/authorize</code> ainda está caindo na nuvem em vez do meu BFF local na porta 8080. Você pode verificar e corrigir?',
    mcp_chat_msg2: 'Identifiquei o problema: não havia uma regra cadastrada para <code>/bff/core/v1/oauth2/authorize</code>, por isso a requisição continuava indo para a nuvem. Vou criar a regra para você agora:',
    mcp_chat_msg3: 'Pronto! Testei novamente e a rota agora retorna <strong>OVERRIDE_LOCAL</strong> direcionando para <code>http://localhost:8080/bff/core/v1/oauth2/authorize</code> com cabeçalhos CORS automáticos.',
    port_tag: 'Alta Disponibilidade',
    port_title: 'Resolução Dinâmica de Portas & Runtime',
    port_desc: 'Conflitos de portas ocupadas são resolvidos automaticamente. Se a porta 8888 estiver em uso, o proxy sobe na próxima livre e sincroniza os clientes.',
    port_c1_title: 'Auto-Switch Inteligente',
    port_c1_desc: 'Se a porta <code>8888</code> estiver ocupada por outro processo ou instância, o BackOverrides varre portas subsequentes (<code>8889</code>, <code>8890</code>...) até encontrar uma livre.',
    port_c2_title: 'Arquivo de Runtime State',
    port_c2_desc: 'O proxy grava o arquivo <code>.back-overrides-runtime.json</code> na raiz do projeto com o PID e a porta exata ativa, permitindo descoberta imediata.',
    port_c3_title: 'Auto-Discovery nos Clientes',
    port_c3_desc: 'O pacote <code>@back-overrides/client</code> e a Extensão do Chrome escaneiam a faixa de portas e se conectam automaticamente à porta ativa sem reconfiguração.',
  ...
  'en': {
    ...
    nav_extension: 'Extension & Studio',
    nav_mcp: 'MCP Server (AI)',
    ...
    sidebar_extension: 'Extension & Studio',
    sidebar_mcp: 'MCP Server & AI',
    sidebar_port_discovery: 'Automatic Port Discovery',
    ...
    ext_tag: 'Zero Code & Studio',
    ext_title: 'Extension for Google Chrome & Edge + BackOverrides Studio',
    ext_desc: 'Network-level interception, visual content-script generator, and 1-click extension reload without opening browser settings.',
    ...
    ext_c4_title: 'BackOverrides Studio',
    ext_c4_desc: 'Visual configuration page (<code>options.html</code>) built into the extension. Create interactive rules, edit regex patterns, and generate the in-page script with 1 click.',
    ext_c5_title: 'Authorization Header Preservation',
    ...
    mcp_title: 'Model Context Protocol (MCP) Server',
    mcp_desc: 'Connect your AI assistant (GitHub Copilot in VS Code, Claude Desktop, Cursor, or Gemini) to BackOverrides to inspect, test, and configure rules via natural language.',
    mcp_vscode_tip: 'Once saved, open <strong>GitHub Copilot Chat</strong> in VS Code and your assistant will have instant access to all BackOverrides tools.',
    mcp_tools_title: 'Tools Exposed by the MCP Server (8 Tools)',
    mcp_tools_desc: 'Your AI assistant gains superpowers to safely inspect and manipulate local network traffic:',
    mcp_tool_status_desc: 'Checks if the local proxy is online, active port (with auto-fallback), PID, and total intercepted request count.',
    mcp_tool_rules_desc: 'Returns the full list of configured routes, allowed HTTP methods, and resolved localhost targets.',
    mcp_tool_add_desc: 'Adds a new rule to <code>back-overrides.json</code> programmatically without manual file editing.',
    mcp_tool_toggle_desc: 'Temporarily enables or disables a rule by index or route to compare traffic with and without interception.',
    mcp_tool_test_desc: 'Simulates how a request (URL + HTTP method) will be handled: rule match, target resolution, and CORS headers.',
    mcp_tool_browser_desc: 'Launches a Chrome, Edge, or Brave instance with the BackOverrides extension pre-loaded.',
    mcp_tool_snippets_desc: 'Generates quick snippets for injection via DevTools console, HTML <code>&lt;script&gt;</code> tag, or npm import.',
    mcp_tool_cfg_desc: 'Automatically configures the <code>.vscode/mcp.json</code> file in the current workspace.',
    mcp_chat_title: 'Practical Example: Dialogue with GitHub Copilot in VS Code',
    mcp_chat_desc: 'See how the assistant uses MCP to diagnose and fix a routing issue in seconds:',
    mcp_chat_msg1: 'Copilot, route <code>/bff/core/v1/oauth2/authorize</code> is still hitting the cloud instead of my local BFF on port 8080. Can you check and fix it?',
    mcp_chat_msg2: 'I identified the issue: there was no rule registered for <code>/bff/core/v1/oauth2/authorize</code>, so requests kept going to the remote cloud. I will create the rule for you now:',
    mcp_chat_msg3: 'All done! I tested it again and the route now returns <strong>OVERRIDE_LOCAL</strong> forwarding to <code>http://localhost:8080/bff/core/v1/oauth2/authorize</code> with automatic CORS headers.',
    port_tag: 'High Availability',
    port_title: 'Dynamic Port Resolution & Runtime',
    port_desc: 'Busy port conflicts are automatically resolved. If port 8888 is in use, the proxy starts on the next available port and syncs clients.',
    port_c1_title: 'Smart Auto-Switch',
    port_c1_desc: 'If port <code>8888</code> is occupied by another process or instance, BackOverrides scans subsequent ports (<code>8889</code>, <code>8890</code>...) until it finds an open one.',
    port_c2_title: 'Runtime State File',
    port_c2_desc: 'The proxy writes the <code>.back-overrides-runtime.json</code> file at the project root with the active PID and exact port for immediate discovery.',
    port_c3_title: 'Client Auto-Discovery',
    port_c3_desc: 'The <code>@back-overrides/client</code> package and Chrome Extension scan the port range and automatically connect to the active port without manual changes.',
